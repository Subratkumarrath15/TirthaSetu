...
parking occupancy, and safety risk levels.

The latest snapshot is:
1. Saved locally to realtime/latest_sensor_data.json
2. Sent to the deployed FastAPI ML service every 3 seconds.
"""
...
# ============================================================================
# DEPLOYED FASTAPI ML SERVICE
# ============================================================================

ML_SERVICE_URL = os.getenv(
    "ML_SERVICE_URL",
    "https://tirthasetu-ml.onrender.com"
)

SENSOR_INGEST_URL = (
    ML_SERVICE_URL.rstrip("/")
    + "/api/realtime/ingest"
)


# ============================================================================
# TEMPLE ZONE CONFIGURATION
# ============================================================================
...
class TempleSensorSimulator:
    ...
    def step(self) -> dict:
        ...
        for temple, zones in ZONE_CONFIGS.items():
            ...
            for zone, cfg in zones.items():
                ...
                # Outflow fluctuates naturally
                outflow_rate = random.uniform(
                    0.85,
                    1.15
                )
        ...
        # Send latest state to deployed FastAPI
        self.send_to_ml_service(snapshot)

        return snapshot


    # ========================================================================
    # SAVE LOCAL SENSOR DATA
    # ========================================================================
    ...
    def save_state(self, snapshot: dict):
        ...
        # Replace previous snapshot atomically
        os.replace(
            temporary_file,
            self.output_file
        )
    ...
    def send_to_ml_service(self, snapshot: dict):
        """
        Send the latest IoT sensor snapshot to the deployed FastAPI service.

        The simulator continues running even if the ML service is temporarily
        unavailable.
        """
        ...
        try:
            ...
            with urlopen(
                ...
                if response.status == 200:
                    ...
                    print(
                        f"[ML SYNC] Cycle #{snapshot['cycle']:03d} "
                        f"successfully sent to FastAPI."
                    )

                else:

                    print(
                        f"[ML SYNC] FastAPI returned "
            ...
            print(
                f"[ML SYNC WARNING] Unable to reach ML service: "
                f"{exc.reason}"
            )
            ...
            print(
                f"[ML SYNC WARNING] Failed to send sensor data: "
                f"{str(exc)}"
            )
    ...
    def print_cycle_summary(self, snapshot: dict):
        ...
        for rec in snapshot["sensors"]:
            ...
            in_out = (
                f"+{rec['people_entering']}"
                f"/-{rec['people_exiting']}"
            )

            cur_pop = (
                f"{rec['current_people']:,}"
            )

            density = (
                f"{rec['crowd_density']:.1f}%"
            )
    ...
    def run(
        ...
        print(
            ">> YATRASAFE REAL-TIME IOT SENSOR SIMULATOR ACTIVE"
        )
        ...
        print(
            f">> State persistence: {self.output_file}"
        )

        print(
            f">> ML Service: {ML_SERVICE_URL}"
        )

        print(
            f">> Telemetry Endpoint: {SENSOR_INGEST_URL}"
        )
        ...
        def sigint_handler(sig, frame):
            ...
            print(
                ...
                "Saving final state and exiting gracefully..."
            )
        ...
        try:
            ...
            while self.running:
                ...
                if (
                    ...
                    print(
                        f"\n[INFO] Reached requested "
                        f"{max_cycles} test cycles. "
            ...
            print(
                f"[SUCCESS] Final simulation state saved "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="YatraSafe IoT Sensor Simulator"
    )
