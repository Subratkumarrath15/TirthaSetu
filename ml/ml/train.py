...
def train_model():
    ...
    if missing_cols:
        print(
            f"[ERROR] Missing required columns in dataset: "
            f"{missing_cols}"
        )
        sys.exit(1)
    ...
    print(
        f"   Training Set: {len(X_train):,} samples (80%)"
    )

    print(
        f"   Testing Set:  {len(X_test):,} samples (20%)"
    )
    ...
    #
    # Memory optimization:
    # - Reduced trees from 200 to 100
    # - Changed n_jobs from -1 to 1
    #
    # This reduces RAM/CPU pressure on Render Free (512 MB).
    pipeline = Pipeline(
        steps=[
            (
                "preprocessor",
    ...
    print(
        "\n4. Training RandomForestRegressor Pipeline "
        "(100 estimators, single worker)..."
    )
    ...
    print(
        ...
        f"{mae:10.2f} visitors"
    )

    print(
        f"   Root Mean Squared Error (RMSE): "
        f"{rmse:10.2f} visitors"
    )

    print(
        f"   R-squared Score (R2):           "
        f"{r2:10.4f} ({r2 * 100:.2f}%)"
    )
    ...
    print(
        f"\n6. Trained Pipeline saved to: "
        f"{model_output_path}"
    )

    # 9. Save Model Metrics JSON
    training_timestamp = datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    print(
        f"   Predicted Visitors:    "
        f"{round(predicted_visitors):,}"
    )

    print(
        f"   Prediction Difference: "
        f"{diff:+.2f} ({pct_diff:.2f}%)"
    )
    ...
    print(
        ">> AI crowd prediction model trained "
